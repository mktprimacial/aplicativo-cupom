...
def create_app(test_config=None):
    ...
    def install():
        ...
        with get_conn(app.config["DATABASE_PATH"]) as conn:
            conn.execute(
                "INSERT INTO oauth_states (state, store_domain) VALUES (?, ?)",
                (state, store_domain),
            )
    ...
    def callback():
        ...
        if not code or not store_id or not store_domain or not state:
            return jsonify({"error": "code, store_id, store_domain and state are required"}), 400
